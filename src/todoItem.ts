...
export default class TodoItem extends React.Component<TodoItemProps, TodoItemState> {
  ...
  handleEdit() {
    this.props.todo.ui.set({ status: Counter.Status.EDITING });
  }

  handleKeyDown(event: KeyboardEvent) {
    if (event.which === Counter.ESCAPE_KEY) {
      this.props.todo.ui.set({ status: Counter.Status.READY });
    } else if (event.which === Counter.ENTER_KEY) {
      this.handleUpdate();
    }
  }
  ...
  componentDidUpdate(prevProps: TodoItemProps, prevState: TodoItemState, prevContext: any) {
    ...
    if (prevProps.todo.ui.status != status && status == Counter.Status.EDITING) {
  ...
  render() {
    var todo = this.props.todo,
      className = Counter.Status[todo.ui.status].toLowerCase(),
      content: React.DOMElement<React.HTMLAttributes, HTMLDivElement> = null;

    if (todo.model.completed)
      className += ' completed';

    if ([Counter.Status.EDITING, Counter.Status.UPDATING].indexOf(todo.ui.status) != -1) {
